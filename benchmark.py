...
class Benchmark(object):
    ...
    def run(self):
        ...
        for experiment_i in tq(range(self.sample_count)):
            ...
            train_acc, test_acc = self.train_and_test(
            ...
            for explain_name in self.METHODS:
                ...
                accs = self.evaluate_explanation(
                    time_wrapper, model, test_dataset, explain_name
                )
                print(
                    f"Benchmark:{benchmark_name} Run #{experiment_i + 1}, Explain Method: {explain_name}, Accuracy: {np.mean(accs)}"
                )
            ...
            print(
                f"Benchmark:{benchmark_name} Run #{experiment_i + 1} finished. Average Explanation Accuracies for each method:"
            )
            accuracies_summary = {}
            for name, run_accs in all_explanations.items():
                run_accs = [np.mean(single_run_acc) for single_run_acc in run_accs]
                accuracies_summary[name] = {
